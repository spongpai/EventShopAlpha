...
public class SystemUtilities 
{
	...
	private static final String QUERYSTORE = Config.getProperty("tempDir") + "/query/";
	private static final String DATASTORE = Config.getProperty("tempDir") + "/datasource/";
	...
	private static boolean isFixDS(String ds){
		String[] fixDS = {"ds0.", "ds1.","ds2.", "ds3.","ds4.", "ds5.","ds6.", "ds7.","ds8.", "ds9.","ds10.", "ds11.","ds12.", "ds13.","ds14."};
	...
	public static int checkOS(){
		...
		int os;
		if( currentOs.indexOf("nix") >= 0 || currentOs.indexOf("nux") >= 0 || currentOs.indexOf("aix") > 0){
	    	os = 0;
	...
	private static void removeTempFile(int os) throws IOException{
		...
		if(ds != null){
			for(int i = 0; i < ds.length; i++){
				if(ds[i].isFile() && !isFixDS(ds[i].getName()) && !ds[i].getName().contains("before")){
		...
		if(objQuery != null){
			for(int i = 0; i < objQuery.length; i++){
				...
				if(m.find() && !objQuery[i].getName().contains("q13") && !objQuery[i].getName().contains("q4")){
		...
		if(pattern != null){
			...
			if(objQuery != null){
				for(int i = 0; i < exeQuery.length; i++){
					...
					if(m.find() && !exeQuery[i].getName().contains("q13") && !exeQuery[i].getName().contains("q4")){
		...
		if(srcQuery != null){
			for(int i = 0; i < srcQuery.length; i++){
				...
				if(m.find() && !srcQuery[i].getName().contains("q13") && !srcQuery[i].getName().contains("q4") ){
		...
		if(os == 0){
			...
			while ((line = reader.readLine()) != null) {
				if (line.contains(PROCESS)){
					...
					Runtime.getRuntime().exec(KILL + " " + line.split(" ")[0] + " /f");
					found = true;
				}
			}
	...
	private static void moveTempFile(int os) throws IOException{
		...
		Date today = new Date();
		String todayTxt =  "" + today.getYear() + (today.getMonth() + 1) + today.getDate();
		...
		if(resultsEmage != null){
			for(int i = 0; i < resultsEmage.length; i++){
				if(resultsEmage[i].isFile()){
					File destFile = new File(queryStoreDir.getAbsolutePath() + "/" + resultsEmage[i].getName() + "_" + resultsEmage[i].lastModified());
					if(destFile.exists())
						destFile.delete();
		...
		if(ds != null){
			for(int i = 0; i < ds.length; i++){
				if(ds[i].isFile() && !isFixDS(ds[i].getName()) && !ds[i].getName().contains("before")){
					File destFile = new File(DataStoreDir.getAbsolutePath() + "/" + ds[i].getName() + "_" + ds[i].lastModified());
					if(destFile.exists())
						destFile.delete();
		...
		if(objQuery != null){
			for(int i = 0; i < objQuery.length; i++){
				Matcher m = r.matcher(objQuery[i].getName());
				if(m.find() && !objQuery[i].getName().contains("q13") && !objQuery[i].getName().contains("q4")){
					File destFile = new File(queryStoreDir.getAbsolutePath() + "/" + objQuery[i].getName() + "_" + objQuery[i].lastModified());
					if(destFile.exists())
						destFile.delete();
		...
		if(pattern != null){
			...
			if(objQuery != null){
				for(int i = 0; i < exeQuery.length; i++){
					Matcher m = r.matcher(exeQuery[i].getName());
					if(m.find() && !exeQuery[i].getName().contains("q13") && !exeQuery[i].getName().contains("q4")){
						File destFile = new File(queryStoreDir.getAbsolutePath() + "/" + exeQuery[i].getName() + "_" + exeQuery[i].lastModified());
						if(destFile.exists())
							destFile.delete();
		...
		if(srcQuery != null){
			for(int i = 0; i < srcQuery.length; i++){
				Matcher m = r.matcher(srcQuery[i].getName());
				if(m.find() && !srcQuery[i].getName().contains("q13") && !srcQuery[i].getName().contains("q4") ){
					File destFile = new File(queryStoreDir.getAbsolutePath() + "/" + srcQuery[i].getName() + "_" + srcQuery[i].lastModified());
					if(destFile.exists())
						destFile.delete();
