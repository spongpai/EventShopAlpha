...
//		// Start the output pipe to output the emage
		...
		while(isRunning)
		{
			//log.info("count:" + count + ", " + isRunning + ", " + it.hasNext());
			while(it.hasNext())
			{
				if(!isRunning) break;
				Emage e = it.next();	
				...
				try {
					...
					// 08/19/2011 Mingyan
					output.close();
					log.info("["+e.emageID+"]  successfully write the (binary) output file " + filepath);
				} catch (IOException e1) {
					log.error(e1.getMessage());
					// add by Siripen, to solve runnning method calling twice from the front UI, cause this locking throw an error
				...
				log.info("["+e.emageID+"]  wrote json file " + count + " emage(s)! " + imgpath);
				
				// to store in mongodb
				/*
				if(db != null) {
					insertToMongoDB(db, e);
					log.info("["+e.emageID+"] store in mongodb ");
				} else {
				...
				*/
				...
				//log.info("["+e.emageID+"]  wrote png overlay " + count + " emage(s)! " + imgpath);
	...
	public void writeJsonEmage(Emage e, String fileName){
		//CommonUtil.writeToFile(fileName + ".json", e.toJson().toString(), false);
		CommonUtil.writeToFile(fileName + ".json", e.toGeoJson().toString(), false);
		CommonUtil.writeToFile(fileName + "_stt.json", e.getSTTPointList().toString(), false);
		//CommonUtil.writeToFile(fileName + "_"+e.startTime.getTime()+".json", e.getAllFormat().toString(), false);
...
//				log.info("create ds image (png) for visulization");
	...
	public static void main(String[] args) {
		...
		FrameParameters fp = new FrameParameters(1*6*60*1000, 0, 2, 2, 24, -125, 50, -66);//CHANGED TO 6 MINUTES for testing...change back to 60 MINUTES !!!!*****
		...
		DataProcess process = new DataProcess(null, EIter, wrapper, tempDir + "99_Twitter-test", imgBasePath + "99", "99");
		...
		DataProcess process2 = new DataProcess(null, EIter2, simWrapper, tempDir + "98_Sim", imgBasePath + "98", "98");
...
//		 srcJSONText =
//		 "{\"srcID\":3,\"source\":{\"theme\":\"Asthma\",\"name\":\"Twitter-Asthma3\",\"url\":\"" + tempDir + "ds3_csvFile\",\"format\":\"csv\",\"supportedWrapper\":null,\"bagOfWords\":[],\"visualParam\":null,\"initRes\":{\"timeWindow\":\"1\",\"syncAtMilSec\":\"1\",\"latUnit\":\"0.1\",\"longUnit\":\"0.1\",\"swLat\":\"24.0\",\"swLong\":\"-125.0\",\"neLat\":\"50.0\",\"neLong\":\"-66.0\"},\"finalRes\":{\"timeWindow\":\"10\",\"syncAtMilSec\":\"1\",\"latUnit\":\"2\",\"longUnit\":\"2\",\"swLat\":\"24\",\"swLong\":\"-125\",\"neLat\":\"50\",\"neLong\":\"-66\"}}}";
