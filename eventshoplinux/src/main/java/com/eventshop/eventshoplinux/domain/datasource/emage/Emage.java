...
public class Emage {
	...
	public void setMinMax(){
		if(image != null){
			max = -999999999;//a large NEGATIVE number which should get overwritten
			min =  999999999;
		...
		for(int i = 0; i < image.length; i++){
			for(int j = 0; j < image[0].length; j++){
				...
				if(!Double.isNaN(image[i][j]) && image[i][j] > 0.0) {
					...
					stt.addProperty("bbox", 
							"[" + point.getX() + "," + point.getY() + "," + (point.getX() + latUnit) + "," + (point.getY() + longUnit) + "]");
					stt.addProperty("result", image[i][j]);
					sttList.add(stt);
				}
			}
...
//         };
	...
	// NOTE!!! for GeoJson format, the coordinate is longitude, latitude  (not lat/long)
	public JsonObject getSTTBoxGeoJson(){
			...
			for(int j = 0; j < image[0].length; j++){
				...
				if(!Double.isNaN(image[i][j]) && image[i][j] > 0.0) {
					...
					geoMetries ="{\"geometry\":{\"type\":\"Polygon\", " +
							"\"coordinates\":[[["+swLong+","+swLat+"],["+swLong+","+(swLat+latUnit)+"],["+(swLong+longUnit)+","+(swLat+latUnit)+"],["+(swLong+longUnit)+","+swLat+"],["+swLong+","+swLat+"]]]}," +
							"\"properties\": {\"theme\": \""+this.theme+"\", \"value\": "+ image[i][j] +"}, \"type\":\"Feature\"},";
		...
		return (JsonObject) parser.parse(geoJsonOpen + sb.toString() + geoJsonEnd);
	 ...
	 * Incorrect format!!!
	public JsonObject getSTTBoxGeoJson(){
		String geoJsonOpen = "{ \"type\":\"FeatureCollection\", \"features\": [{\"geometry\":{\"type\":\"GeometryCollection\",\"geometries\":[";
		String geoMetries = "";
		String geoJsonEnd = " ]},\"type\":\"Feature\",\"properties\":{}}]}";
		...
		for(int i = 0; i < image.length; i++){
			for(int j = 0; j < image[0].length; j++){
				...
				if(!Double.isNaN(image[i][j]) && image[i][j] > 0.0) {
					...
					double swLat = point.getX();
					double swLong = point.getY();
					geoMetries += "{\"type\":\"Polygon\", \"coordinates\":[[["+swLong+","+swLat+"],"
									+ "["+swLong+","+(swLat+latUnit)+"],"
									+ "["+(swLong+longUnit)+","+(swLat+latUnit)+"],"
									+ "["+(swLong+longUnit)+","+swLat+"]]]},";
				}
			}
		}		
		geoMetries = (geoMetries == null || geoMetries.length() == 0?geoMetries:geoMetries.substring(0, geoMetries.length() - 1));
		JsonParser parser = new JsonParser();
		//System.out.println(geoJsonOpen + geoMetries + geoJsonEnd);
		return (JsonObject) parser.parse(geoJsonOpen + geoMetries + geoJsonEnd);
		
	}
	*/
	
	// Convert pixel to latitude and longitude (the point at the lower left corner)
	...
	public SpatialBound pixel2geoBound(Point2D pp)
	{
		...
		sb.sw.setLocation(((pp.getX()-swLat)*latUnit), ((pp.getY()-swLong)*longUnit));
		sb.ne.setLocation(((pp.getX()+1-swLat)*latUnit), ((pp.getY()+1-swLong)*longUnit));
		return sb;
	}
...
//	}
	...
	public static void main(String[] args){
		FrameParameters fp = new FrameParameters(1*6*60*1000, 0, 2, 2, 24, -125, 50, -66);
