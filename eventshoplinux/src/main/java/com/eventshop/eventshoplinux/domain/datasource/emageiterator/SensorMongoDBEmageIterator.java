...
{
	protected static Log log=LogFactory.getLog(SensorMongoDBEmageIterator.class);
	String fileURL;	
	MongoDB mongo;
	double radious;
	...
	public SensorMongoDBEmageIterator (FrameParameters fp, String th, String url, MongoDB mongo, long startTimeStream, long endTimeStream, boolean endTimeUnbound, double radious)
	{
		params = fp;
		this.theme =th;
		this.fileURL = url;
	...
	public SensorMongoDBEmageIterator (FrameParameters fp, String th, String url, MongoDB mongo, long startTimeStream, long endTimeStream, boolean endTimeUnbound, double radious, DatasourceType ds, SpatialWrapper sw){
		params = fp;
		this.theme =th;
		this.fileURL = url;
		this.mongo = mongo;
	...
	public void run() 
	{
		...
		while(isRunning)
		{
			Emage e;
			try {
				long startEit = this.startTimeStream + (numEmage * params.timeWindow);
			...
			for(double j = params.swLong; j < params.neLong; j = j+params.longUnit)
			{
				...
				if(sensorList.size() == 0 && isPredict){
					...
					while(sensorList.size() == 0){
						pr = pr*2;
						String qstr2 = "{$and:[{date:{$gt:'"+formatter.format(new Date(start))+"',$lt:'"+formatter.format(new Date(end))+"'}},"
								+ "{'loc':{$geoWithin:{$centerSphere:[["+lng+","+lat+"],"+pr+"]}}}]}";
				...
				for(DBObject sensor: sensorList){
					...
					System.out.println(value + "," + sensorDate + ",long/lat: " + sensroLng+"/"+sensorLat);
	...
	public static void main(String[] args) {
		try {
			/*
			long timeWindow = 1000*60*24; //1mins for testing//*60*24*7; // the last 7 days
			long sync = 1000; 
			double latUnit = 2;
			double longUnit = 2;
			double swLat = 20;
			...
			FrameParameters fpSoCal = new FrameParameters(1000*60*60*24, 0, 0.2, 0.2, 37.60, -122.40, 40.00, -120.80);
			...
			mongo.setCollection("PM2_5_daily");
			SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
			
			Date st = formatter.parse("2013-08-10");
			Date et =formatter.parse("2013-12-31");
			long startTimeStream = st.getTime();
			long endTimeStream = et.getTime();

			SensorMongoDBEmageIterator sensorEit = new SensorMongoDBEmageIterator(fpSoCal, theme, tempFile, mongo, startTimeStream, endTimeStream, false, 10.0, DatasourceType.point,SpatialWrapper.sum);
			//sensorEit.run();
			...
			*/
			...
			DataSource src = new DataSource(id, theme, name, url, format, supported, null, null,fpSoCal, null);
			...
			SensorMongoDBEmageIterator mongoEIter = new SensorMongoDBEmageIterator(src.initParam, src.srcTheme, src.url, mongo, startTimeStream, endTimeStream, false, 10.0, DatasourceType.point,SpatialWrapper.max);
			mongoEIter.setPredict(predict);
			mongoEIter.setSrcID(Long.parseLong(src.srcID));
			STMerger merger = null;
			DataProcess process = new DataProcess(merger, mongoEIter, null, filepath, imgBasePath+src.srcID, src.srcID);
