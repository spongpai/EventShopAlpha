...
public class AlertProcessor 
{
	...
	{
		try {
		this.SocialDataSource = registerServlet.context+Constant.RESULT_DS+userDS+"_before.png"; 
		...
		if(userDS.contains("ds"))
		{
			...
			{
				if(((DataSource)(registerServlet.sources.get(i))).srcID.equals(userDS.substring(2)))	{
					this.theme= ((DataSource)registerServlet.sources.get(i)).srcTheme;
				}
			}
		}
		...
		if(srcQuery.contains("ds"))
		{
			...
			for(int i = 0; i < registerServlet.sources.size(); ++i)
			{
				if(((DataSource)(registerServlet.sources.get(i))).srcID.equals(userDS.substring(2)))
				{
					this.params = ((DataSource)registerServlet.sources.get(i)).finalParam;
					this.probEmagePath =  registerServlet.context+Constant.RESULT_DS+srcQuery+"_before.png";
				}
			}
		}
		...
		if(srcQuery.contains("Q"))
		{
			if(registerServlet.queryProcesses.containsKey(srcQuery)){
				...
				this.probEmagePath =  registerServlet.context+Constant.RESULT_Q+((QueryProcess)registerServlet.queryProcesses.get(srcQuery)).outputName+"_before.png";
		...
		if(nearestDS.contains("ds"))
		{
			...
			for(int i = 0; i < registerServlet.sources.size(); ++i)
			{
				if(((DataSource)registerServlet.sources.get(i)).srcID.equals(nearestDS.substring(2)))
				{
					this.params = ((DataSource)registerServlet.sources.get(i)).finalParam;
					this.solEmagePath =  registerServlet.context+Constant.RESULT_DS+nearestDS+"_before.png";
				}
			}
		}
		...
		if(nearestDS.contains("Q"))
		{
			...
			if(registerServlet.queryProcesses.containsKey(nearestDS)){
				...
				this.solEmagePath =  registerServlet.context+Constant.RESULT_Q+((QueryProcess)registerServlet.queryProcesses.get(nearestDS)).outputName+"_before.png";
		...
		// Ev3ntResearcher 
		.setOAuthConsumerKey("ZZ1tXcer3MnaT2IDHHVw")
		.setOAuthConsumerSecret("jQow0Onsn2zXTEHlnwyxlQjFpLbQmT2lnqTeVtBGQ8")
		.setOAuthAccessToken("400474501-yvrmXdZGLCQtusjMC7nuU7zuPsVgLP8r1cG0a2bN")
		.setOAuthAccessTokenSecret("BiSE0n5nJkGOAVPXVWsS9nTrlwhlxlyG2v99AsDCYg");
	...
	public void  DoAlerts()
	{
		...
		PlanarImage probEmage = JAI.create("fileload", probEmagePath);//dummy loaded. date to be edited.
		...
		for (int i=0; i<lengthProb; i++)
		{
			...
			{
				if (probDataGrid[i][j] > minValProb  && probDataGrid[i][j] < maxValProb)
					probDataGrid[i][j] = 1;
				else
					probDataGrid[i][j] = 0;
			}
		...
		PlanarImage solEmage = JAI.create("fileload", solEmagePath);//dummy loaded. date to be edited.
		...
		for (int i=0; i<lengthsol; i++)
		{
			...
			{
				if (solDataGrid[i][j]>minValSol && solDataGrid[i][j]<maxValSol)
					solDataGrid[i][j] = 1;
				else
					solDataGrid[i][j] = 0;
			}
		...
		int maxDistPoss = (int)Math.sqrt(lengthProb*lengthProb+widthProb*widthProb);
		...
		for (int i=0; i<lengthProb; i=i+ (int)(2/params.latUnit))
		{
			for (int j=0; j<widthProb; j=j+ (int) (2/params.longUnit) )
			{
				if (probDataGrid[i][j]==1)
				{
					...
					neighbFound = false;
					for (int step=0; step<Math.min(maxDistPoss,40) && !neighbFound; step++)
					{
					...
					}//end of for for loop for finding neighs... i,e neighbFound=true now OR npoint =(-999,-999)
					
					writeToFile(" neighbFound=" + neighbFound + ", npoint=" + npoint.x + npoint.y + ", " + minValSol + ", " + maxValSol);
					...
					if (npoint.x!=-999 && npoint.y!=-999)
					{
						...
						Point2D nPointGeoNN = pixel2LatLong(npoint.x, npoint.y, params);
						String tweetText="";
						...
						int nPointGeoLat = (int)params.swLat+(int)(Math.floor((nPointGeoUser.getX()-params.swLat)/2)*2)+1;//THIS IS HARDCODED WITH ASSUMPTION of TWItter initRes ==2 units
						int nPointGeoLong = (int)params.swLong+(int)(Math.floor((nPointGeoUser.getY()-params.swLong)/2)*2)+1;

						String userNames = "";
						try {
							userNames = search(startDate, nPointGeoLat+"", nPointGeoLong+"", theme);
						} catch (Exception e){
							writeToFile(e.getMessage());
							log.error(e.getMessage());
						}
						...
						{
							StringTokenizer users = new StringTokenizer(userNames, " ,");
							int numUsers = Math.min(3, users.countTokens());//limiting to 5 per lat long block
							for (int un=0; un<numUsers; un++)
							{
								
								String addressFound=getShelterAddr(nPointGeoNN.getX(), +nPointGeoNN.getY());
								if (un==0 && addressFound.length()>10)
								tweetText = "(TEST ONLY): Be safe from hurricane http://goo.gl/2l4OT. Shelter near you: " + addressFound;
								if (un==1 && addressFound.length()>10)
									tweetText = "Be safe from hurricane http://goo.gl/2l4OT .(TEST ONLY).Shelter near you: " + addressFound;
								if (un==2 && addressFound.length()>10)
									tweetText = "Be safe! http://goo.gl/2l4OT .(TEST ONLY). Hurricane shelter near you: " + addressFound;
								if (un==3 && addressFound.length()>10)
									tweetText = "(TEST ONLY) Be safe in Hurricane! http://goo.gl/2l4OT .Hurricane shelter near you: " + addressFound;
								if (un==4 && addressFound.length()>10)
									tweetText = "Hurricane shelter near you: " + addressFound+"(TEST ONLY). http://goo.gl/2l4OT Be safe in Hurricane!";
									
								//For each user in each effected location; send out the tweet
								
								//send out tweet for each user
								String userid=users.nextToken();
								while ((usersTweeted.contains(userid)) && users.hasMoreTokens())
								{
									userid=users.nextToken();
								}
								// for testing
								String tweetOut ="@ "+ userid+" "+tweetText;
								...
								log.error("Successfully updated the status to [" + status.getText() + "].");
							}
							try {
								Thread.sleep(60000);
						...
						{
							log.info("No users found for location:" +nPointGeoLat +","+nPointGeoLong);
							allMsg += " No users found for location:" +nPointGeoLat +","+nPointGeoLong + "++++++++++++++++++ ";
						}
						writeToFile("All messages: " + allMsg);
						
					}//else no valid near neighbor locations found for sending alerts
					else {
	...
	public void writeToFile(String msg){
		try{
			// Create file /
			FileWriter fstream = new FileWriter(Config.getProperty("tempDir") + "file_log.txt", true);
	...
	public String getShelterAddr(double lat, double lng){
		...
		sql += " lat>=" + minLat + " and lat <= " + maxLat + " and lng >= " + minLng + " and lng <= " + maxLng + " limit 1";
		...
		try {
			...
			if(rs.next()){
				address = rs.getString("addr1") + ", " + rs.getString("city") + ", " + rs.getString("state") + " " + rs.getString("zip");
			} else {
	...
	public String search(String dateStart, String Lat, String Lon, String theme) throws Exception
	{
		String result = "";
		String sql; 
		...
		sql = "SELECT userName FROM tbl_"+theme+"_tweet" +" WHERE " + dateCond + " AND " + locCond;
		log.info(sql);
		writeToFile(sql);
		try { 
	...
	public void connection()
	{
		try {
			...
			conn = DriverManager.getConnection(url, Config.getProperty("DBusr"),Config.getProperty("DBpwd"));
