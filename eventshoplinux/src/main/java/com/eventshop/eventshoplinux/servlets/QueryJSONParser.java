...
//	}
	...
	public QueryProcess parseQuery(int qMasterId, List<String> queryTree, FrameParameters finalFrame)
	{
	...
	private String parseAggregation(JsonObject query)
	{
		...
		for(int i = 0; i < srcArray.size(); i++)
		{
			...
			if(dataSrc.indexOf("ds") != -1)
			{
				...
				{
					// If the data source has been added once, then ignore it
					System.out.println("compare source id, srcId: " + srcID + ", " + newQuery.queriedSources.get(j).srcID);
					
					if(srcID.equalsIgnoreCase(newQuery.queriedSources.get(j).srcID))
					{
						srcFound = true;
						// Change the data Src to be the dsID_subscript
				...
				if(!srcFound)
				{
					System.out.println("add new ds to the queriedSources " + srcID);
					newQuery.queriedSources.add(servlet.sources.get(srcID));
					//newQuery.setFinalResolution(servlet.getFinalParamFromSrc(srcID));
				}
			}
	...
	public static void main(String[] args)
	{
		...
		int qMasterId = 7;
		QueryProcess q = parser.parseQuery(qMasterId, qdao.getQueryTree(qMasterId), qdao.getFrameParameterQry(qMasterId));
		q.build();
