...
public class AdminService {
	...
	public User[] getUsers() {
		...
		users = list.toArray(users); // ui finds it difficult to read unless the list is converted to array
		return users;
	}
	...
	public String createUsers(User[] users) {
		...
		for(User user : users) {
			result = (eventShopDAO.saveUser(user).equals(SUCCESS)?result:FAILURE); // want to indicate overall failures if any going forward
		}
		return result;
	}
	 ...
	 */
	public HashMap viewUserDatasources( @QueryParam("selectedUserIds")
			 String ids) {
		AdminManagementDAO adminDAO = new AdminManagementDAO();		
...
//	}
	...
	public List getDSUsers(
		...
		for(int i=0;i<dsUsers.size();i++) {			
			userIds = new StringBuffer().append(userIds).append(dsUsers.get(i).getId()).append(',').toString();
		}
		
		userIds = userIds.substring(0, userIds.length()-1); // need to check length		
