...
public class AdminManagementDAO extends BaseDAO {
	 ...
	 * selected users
	 */
	public HashMap<Integer, Object> getDataSrcQryLst(
		...
		try {
			...
			for (int i=0;i<selectedUserIdList.length;i++) {
				int userId = Integer.parseInt((String) selectedUserIdList[i]);	
	...
	public String getUserDataSourceName(int userId) {
		...
		try {
			...
			while (rs.next()) {
				dsNames = new StringBuffer().append(dsNames).append(rs.getString(dsmaster_title)).append(COMMA).toString();
			
			}
			dsNames = (dsNames.equals(EMPTY_STRING)?null:dsNames.substring(0,dsNames.length()-1));
		} catch (Exception e) {
	...
	public String getUserQueryName(int userId) {
		...
		try {
			...
			while (rs.next()) {
				qryNames = new StringBuffer().append(qryNames).append(rs.getString(1)).append(COMMA).toString();

			}
			qryNames = (qryNames.equalsIgnoreCase(EMPTY_STRING)?null:qryNames.substring(0,qryNames.length()-1));
	...
	public List<DataSourceListElement> getDataSrcList() {
		...
		try {
			...
			while (rs.next()) {
				...
				dsElement.setTimeWindow(rs.getLong(8)); // need to set timeWindow for registerServlet -- sanjukta 06-08-2014
	...
	public String deleteDS(List<String> dsIds) {
		...
		while (listIterator.hasNext()) {
			...
			String statusDSRes = deleteReferences(DS_TABLE_NAME,DS_COL_NAME,dsId);
			String statusWrap = deleteReferences(WRP_TABLE_NAME,WRP_COL_NAME,dsId);
	...
	public List<DataSource> getDatasource(List<String> selectedDSIds){
		...
		try {
			...
			ps.setInt(1, dsId);
			res = ps.executeQuery();
			//SLT_DSMSTR_RUN_QRY = "SELECT dsm.dsmaster_title,dsm.dsmaster_theme,dsm.dsmaster_url,dsm.dsmaster_format,wr.wrapper_name,wr.bag_of_words FROM Datasource_Master dsm,Wrapper wr WHERE dsm.dsmaster_id=wr.wrapper_id AND dsm.dsmaster_id=?";
			...
			while (res.next()) {
				...
				if (VISUAL.equals(res.getString(4))) {
					String filepathTransMatrix = tempDir + PATH_DS + dsId + UNDERSCORE + TRANSMATRIX;
					String filepathColorsMatrx = tempDir + PATH_DS + dsId + UNDERSCORE + COLRSMATRIX;
					...
					try {
						...
						System.out.println("visual_trans_mat does not exist"+e);
					}
					try {
						if(is != null) {
						...
						System.out.println("visual_colr_mat does not exist"+e);
					}
